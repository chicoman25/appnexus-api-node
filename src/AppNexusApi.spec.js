...
describe(__filename, () => {
  ...
  describe('login Tests', () => {
    ...
    it('returns a token upon successful auth', async () => {
      axios.post.mockResolvedValue({ status: 200, data: successfulAuthResponse });
      const response = await appNexus.login("user", "pass");
    ...
    it('throws an error when auth fails with a response of too many tries', async () => {
      axios.post.mockResolvedValue({status: 200, data: tooManyTriesResponse});
      await expect(appNexus.login("user", "pass")).rejects.toThrowError();   
    });
  });
